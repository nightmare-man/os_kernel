...
;-----set_cursot应该是原子操作 不然对高位和低位的操作可能有误 如A线程已经写入了高位 发出了写低位命令，但是还没写入，被中断
;b线程发出了写命令 待写入高位，再次被中断 换到A,这时A原本计划写低位，结果写的被当作了高位，因此光标越界，访问显存越界 超过了
;段描述符表里的界限 GERNERAL PROTECTION
.set_cursor:;将光标设置位bx
mov dx,0x03d4
mov al,0x0e
out dx,al
